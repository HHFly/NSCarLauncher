...
//        }
    ...
    protected void showVolumDialog(float deltaY) {
        ...
        if (mVolumeDialog == null) {
            ...
            mVolumeDialog.setOnDismissListener(new DialogInterface.OnDismissListener() {
                @Override
                public void onDismiss(DialogInterface dialog) {
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            JCFullScreenActivity.myHandler.sendEmptyMessage(JCFullScreenActivity.HIDE);

                        }
                    }).start();

                    if(IsUp&&mDownY<200) {
                        JCFullScreenActivity.myHandler.sendEmptyMessage(JCFullScreenActivity.SHOW);
                    }
                }
            });
        }
